...
local function ReadEVENT_ReplicateEquipWeapon(): ({ { Player: Player, GunInstanceId: number, Equip: boolean } })
	-- Read BLOCK: 1 bytes
	...
	if Length > 20 then error(`Expected "Length" to be smaller than 20, got {Length} instead.`) end
	...
	for Index = 1, Length do
		...
		ARRAY_START_1 += 2
		Item_1.GunInstanceId = buffer.readu16(RecieveBuffer, OPERATION_OFFSET_0)
		-- Read 1
		local OPERATION_OFFSET_1 = ARRAY_START_1
		ARRAY_START_1 += 1
		Item_1.Equip = (buffer.readu8(RecieveBuffer, OPERATION_OFFSET_1) == 1)
...
local function ReadEVENT_FireSound(): (number, Vector3, Vector3, number, number)
	-- Read BLOCK: 29 bytes
	...
	local Encoded = buffer.readu16(RecieveBuffer, BLOCK_START + 25)
	local Value4
	...
	elseif MantissaExponent == 0b0_00000_0000000000   then
		...
		Value4 = if Encoded // 0x8000 == 1 then -Result else Result
	end
	local Value5 = buffer.readu16(RecieveBuffer, BLOCK_START + 27)
	return Value1, Value2, Value3, Value4, Value5
end

local function WriteEVENT_FireWeapon(Value1: number, Value2: { Origin: Vector3, Direction: Vector3, Time: number, ShotIndex: number, LastShootTime: number }): ()
	...
	buffer.writef32(SendBuffer, BLOCK_START + 26, Value2.Time)
	buffer.writeu16(SendBuffer, BLOCK_START + 30, Value2.ShotIndex)
	buffer.writef32(SendBuffer, BLOCK_START + 32, Value2.LastShootTime)
end

local function WriteEVENT_ReloadWeapon(Value: number): ()
...
local function ReadEVENT_SyncEventState(): (number, number, number)
	-- Read BLOCK: 4 bytes
	local BLOCK_START = Read(4)
	local Value1 = buffer.readu8(RecieveBuffer, BLOCK_START + 0)
	local Value2 = buffer.readu8(RecieveBuffer, BLOCK_START + 1)
	local Value3 = buffer.readu16(RecieveBuffer, BLOCK_START + 2)
...
local function WriteEVENT_ChangeStateWeapon(Value1: number, Value2: number, Value3: boolean): ()
	...
	buffer.writeu8(SendBuffer, BLOCK_START + 3, Value3 and 1 or 0)
end
...
Unreliable.OnClientEvent:Connect(function(Buffer: buffer, Instances: {Instance})
	...
	while (RecieveCursor < Size) do
		...
		local Index = buffer.readu8(RecieveBuffer, BLOCK_START + 0)
		if Index == 0 then
			local Value1: number, Value2: Vector3, Value3: Vector3, Value4: number, Value5: number = ReadEVENT_FireSound()
			if Events.Unreliable[0] ~= nil then
				task.spawn(Events.Unreliable[0], Value1, Value2, Value3, Value4, Value5)
			end
		end
	end
end)
...
local Returns = table.freeze({
	...
	ReplicateEquipWeapon = {
		On = function(Listener: (Value: { { Player: Player, GunInstanceId: number, Equip: boolean } }) -> ()): () -> ()
	...
	FireSound = {
		On = function(Listener: (Value1: number, Value2: Vector3, Value3: Vector3, Value4: number, Value5: number) -> ()): () -> ()
	...
	FireWeapon = {
		Fire = function(Value1: number, Value2: { Origin: Vector3, Direction: Vector3, Time: number, ShotIndex: number, LastShootTime: number }): ()
			local Previous = Save()
			Load()
	...
	SyncEventState = {
		On = function(Listener: (Value1: number, Value2: number, Value3: number) -> ()): () -> ()
	...
	ChangeStateWeapon = {
		Fire = function(Value1: number, Value2: number, Value3: boolean): ()
			local Previous = Save()
			Load()
