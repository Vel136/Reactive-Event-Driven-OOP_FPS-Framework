...
-- MinHeap.lua (OPTIMIZED)
--[[
	Manages a minimum heap data structure including:
...
--[=[
	...
	A minimum heap implementation with O(log n) insertion and extraction.
	Elements are ordered such that the smallest element is always at the root.
	
	Supports custom comparison functions for complex data types.
	OPTIMIZED: O(log n) removal with element-to-index tracking.
]=]
local Identity = "MinHeap"
local MinHeap = {}
...
function MinHeap.GetSize(self : MinHeap): number
	return #self._heap
end
...
--[[
	...
	@return any? - Minimum element or nil if empty
]]
function MinHeap.Peek(self : MinHeap): any?
...
function MinHeap.GetParentIndex(self : MinHeap, index: number): number
...
function MinHeap.GetLeftChildIndex(self : MinHeap, index: number): number
	return index * 2
end
...
function MinHeap.GetRightChildIndex(self : MinHeap, index: number): number
	return index * 2 + 1
end
...
function MinHeap.HasParent(self : MinHeap, index: number): boolean
...
function MinHeap.HasLeftChild(self : MinHeap, index: number): boolean
...
function MinHeap.HasRightChild(self : MinHeap, index: number): boolean
...
function MinHeap.Compare(self : MinHeap, a: any, b: any): boolean
...
function MinHeap.Swap(self : MinHeap, indexA: number, indexB: number)
...
function MinHeap.HeapifyUp(self : MinHeap, index: number?)
...
function MinHeap.HeapifyDown(self : MinHeap, index: number?)
	local currentIndex = index or 1

	while self:HasLeftChild(currentIndex) do
		local smallerChildIndex = self:GetLeftChildIndex(currentIndex)

		if self:HasRightChild(currentIndex) then
			local rightChildIndex = self:GetRightChildIndex(currentIndex)
			if self:Compare(self._heap[rightChildIndex], self._heap[smallerChildIndex]) then
				smallerChildIndex = rightChildIndex
			end
		end

		if self:Compare(self._heap[currentIndex], self._heap[smallerChildIndex]) then
			break
		else
...
function MinHeap.Insert(self : MinHeap, element: any)
	if element == nil then
...
function MinHeap.ExtractMin(self : MinHeap): any?
	if self:IsEmpty() then
		Logger:Warn("ExtractMin: Heap is empty")
		return nil
	end

	local minElement = self._heap[1]
	local lastElement = table.remove(self._heap)
	local oldSize = self:GetSize() + 1

	-- Remove from index map
	if self._indexMap then
		self._indexMap[minElement] = nil
	end
	...
	self.Signals.OnExtract:Fire(minElement, self:GetSize())

	Logger:Print(string.format("ExtractMin: Extracted minimum element, heap size (%d -> %d)", oldSize, self:GetSize()))
...
function MinHeap.Remove(self : MinHeap, element: any): boolean
...
function MinHeap.Clear(self : MinHeap)
...
function MinHeap.GetElements(self : MinHeap): {any}
	local elements = {}
	for i = 1, self:GetSize() do
...
--[[
	...
	Gets elements in sorted order (non-destructive)
	@return table - Sorted array of elements
]]
function MinHeap.GetSortedElements(self : MinHeap): {any}
	...
	while not self:IsEmpty() do
		table.insert(sorted, self:ExtractMin())
	end
...
function MinHeap.ValidateHeap(self : MinHeap): boolean
...
--[[
	...
	@return table - {Size, Min, IsEmpty}
]]
function MinHeap.GetState(self : MinHeap)
	return {
		Size = self:GetSize(),
		Min = self:Peek(),
...
function MinHeap.EnableIndexTracking(self : MinHeap)
	if self._indexMap then
...
function MinHeap.DisableIndexTracking(self : MinHeap)
...
function MinHeap.Destroy(self : MinHeap)
	Logger:Print("Destroy: Cleaning up MinHeap")
...
function MinHeap.GetMetadata(self : MinHeap)
	return self._metadata
end

function MinHeap.SetMetadata(self : MinHeap, metadata)
	self._metadata = metadata
end

local module = {}

local metatable = {__index = MinHeap}

--[[
	Default comparator for numbers (min heap)
	@param a - First value
...
function module.new(comparator: ((any, any) -> boolean)?, metadata: any, enableIndexTracking: boolean?)
	local self : MinHeap = setmetatable({}, metatable)
	...
	Logger:Debug("Created new MinHeap" .. (enableIndexTracking and " with index tracking" or ""))
	return self
end

type Signal = Signal.Signal

export type MinHeap = typeof(setmetatable({}, metatable)) & {
	_heap: {any},
